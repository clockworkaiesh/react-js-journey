        ...
        <div className="restaurant-grid">
          {
            ...
            <>{Array(10).fill("").map((e, index) => (<SkeletonFoodCard key={index}/>))}</>
            : 
            filteredRestaurants?.length === 0 ? <div className="page-container"><h1>No restaurants found</h1></div> :
            filteredRestaurants.map((restaurant) => {
              return(
                <Link className='classic-link' to={"/restaurant/" + restaurant.data.id}  key={restaurant.data.id}>
